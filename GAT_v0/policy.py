...
class NCMultiAgentPolicy(Policy):
    ...
    def _init_net(self):
        ...
        self.gat_layer = GraphAttention(in_features=3*self.n_fc, out_features=3*self.n_fc, dropout=0.0, alpha=0.2)
    ...
    def _run_comm_layers(self, obs, dones, fps, states):
        ...
        for t, (x, p, done) in enumerate(zip(obs, fps, dones)):
            ...
            # Create adjacency matrix from neighbor_mask and add self-connections
            adj = torch.tensor(self.neighbor_mask, dtype=torch.float32).cuda()
            diag = torch.eye(adj.size(0), dtype=adj.dtype, device=adj.device)
            adj = adj + diag  # Add self-connections
            
            # Apply GAT
            s_all = self.gat_layer(s_all, adj)
            
            # Use the GAT-processed features to update LSTM states
            for i in range(self.n_agent):
